...
def discover_sources(topic, depth):
    ...
    search. If the live search fails (network issue, no results, etc.),
    this does NOT substitute fabricated demo-source URLs -- it returns
    an empty source list with a clear "insufficient live sources" error
    message instead, so the app never presents demo content as if it
    were real. The app stays graceful either way: no exception escapes
    this function.
...
def generate_report(topic, depth, plan, sources, analysis, synthesis, sources_are_real=False):
    ...
    if sources_are_real and ai_powered:
        ...
        lines.append(
            "> 🤖 **AI-Assisted, No Live Sources** — Live web search found no "
            "usable sources for this topic, so no sources are listed below. "
            "Analysis and synthesis were still generated by OpenRouter AI."
        )
    else:
        lines.append(
            "> ⚠️ **DEMO MODE** — Live web search found no usable sources for "
            "this topic, and OpenRouter AI was unavailable, so demo analysis "
            "data was used and no sources are listed below."
        )
    lines.append("")
    ...
    if sources_are_real:
        ...
        lines.append(
            "*⚠ Insufficient live sources — a live web search for this topic "
            "did not return any usable results, so no sources are listed here. "
            "No demo/placeholder URLs are shown in their place.*"
        )
    lines.append("")
